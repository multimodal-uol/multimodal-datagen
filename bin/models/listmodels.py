...
from util.constants import MODEL_EXTENSION, MODEL_FILE_REGEX
...
def add_model_info_to_lookup_info(lookup_info):
    ...
    options[
        ...
        algo_name
    )  # can't use the "algo_name" inside options because it may not be present in pre 2.3 models
    options[
        'model_name'
    ] = (
        model_name
    )  # can't use the "model_name" inside options because it may be inconsistent with the model file name
