...
def loglikelihood(R, R_n, variance, M, K):
    ...
    if 0 <= variance <= EPS:
        ...
        res = (
            R_n
            * (np.log(R_n) - np.log(R) - 0.5 * (np.log(2 * np.pi) + M * np.log(variance) + 1))
            + 0.5 * K
        )
        if res == np.inf:
            res = 0
...
def get_additonal_k_split(K, X, clst_labels, clst_centers, n_features, K_sub, k_means_args):
    ...
    for clst_index in range(K):
        ...
        if clst_size <= K_sub:
            ...
            continue
        clst_variance = np.sum((clst_points - clst_centers[clst_index]) ** 2) / float(
            clst_size - 1
        )
        bic_before_split[clst_index] = loglikelihood(
            clst_size, clst_size, clst_variance, n_features, 1
        ) - clst_n_params / 2.0 * np.log(clst_size)
        ...
        for subclst_index in range(K_sub):
            ...
            if subclst_size <= K_sub:
                ...
                continue
            subclst_variance = np.sum(
                (subclst_points - subclst_centers[subclst_index]) ** 2
            ) / float(subclst_size - K_sub)
            log_likelihood = log_likelihood + loglikelihood(
                clst_size, subclst_size, subclst_variance, n_features, K_sub
            )
...
class _XMeans(sklearn_KMeans):
    ...
    def fit(self, X, y=None):
        ...
        while not stop_splitting and iter_num < self.max_iter:
            ...
            add_k = get_additonal_k_split(
                K, X, clst_labels, clst_centers, n_features, K_sub, self.k_means_args
            )
            K += add_k
